...
var addLeftTabs = function(options) {
        ...
        function init($menu_ul, data, level) {
            $.each(data, function(i, item) {
                ...
                if (item.type == undefined) {
                    ...
                    //如果不是header
                    var li = $('<li class="treeview left-treeview  left-' + item.type + '   ' + item.is_show + '" data-level="' + level + '"></li>');
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    } else if (item.targetType != null && item.targetType === "top-nav") { //代表根据顶部导航切换左侧菜单栏
                        var href = 'addLeftTabs({id:\'' + item.id + '\',title: \'' + item.text + '\',close: true,url: \'' + item.url + '\',mark: \'' + item.mark + '\'});';
                        $a.attr('onclick', href);
